...
const Employee_AddNewTask = () => {
    ...
    // for employee
    const [selectedEmpolyee, setselectedEmpolyee] = useState([2]);
    ...
    const FormSubmitTask = (evt) => {
        ...
        data.append('task_labels[]', $("input[name=task_labels]").val());
        ...
        axios.post(Globalsettings.url + 'api/member/task/all-tasks/store/'+companyid+'/'+id, data).then((response) => {
            ...
            setLoading(false);
            setTimeout(() => { 
                history.push(`${process.env.PUBLIC_URL}/employee_task`);
            }, 3000);
        })
        .catch((error) => {
            setLoading(false);
        ...
        <>
            ...
            <div className="card card_dashboard card-body mb-4 whitecolorbg">
                ...
                <div className="col-xl-12 col-lg-12 mb-4">
                    ...
                    <Select name="task_labels" options={labellist.labellist_Array.map((val) => { return({ label: val.label_name, value: val.id }) })} components={animatedComponents} isMulti />
                </div>
                <div className="row">
                ...
                <div className="col-xl-12 col-lg-12 mb-4">
                    ...
                    <Form.Check type="radio" name="priority" label="High" value="high" required checked={priority == "high"} onClick={(e) => setpriority(e.target.value)} />
                    <Form.Check type="radio" name="priority" label="Medium" value="medium" required checked={priority == "medium"} onClick={(e) => setpriority(e.target.value)} />
                    <Form.Check type="radio" name="priority" label="Low" value="low" required checked={priority == "low"} onClick={(e) => setpriority(e.target.value)} />
                </div> 
...
export default Employee_AddNewTask;
