...
// 
import DiscussionListLoop from "../../Project-Tab/Contract_Page_content/DiscussionListLoop";
import DiscussionListLoop_Array from "../../Project-Tab/Contract_Page_content/DiscussionListLoop_Array";
import InnerTabDataLoop from "../../Project-Tab/Contract_Page_content/InnerTabDataLoop";
import InnerTabDataLoop_Array from "../../Project-Tab/Contract_Page_content/InnerTabDataLoop_Array";
...
const Contract_View = (props) => {
    ...
    const formsubmit = (e) => {
        e.preventDefault();
        setLoading(true);
        axios.post(Globalsettings.url + 'api/admin/contracts/add-discussion/'+companyid+'/'+userid+'/'+props.match.params.id,{
            "message" : message,
    ...
    const updatesubmit = (e) => {
        ...
        axios.post(Globalsettings.url + 'api/admin/contracts/update-discussion/'+companyid+'/'+userid+'/'+updatemessageid,{
        ...
        const DeleteMessage = (id) => {
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        axios.get(Globalsettings.url + 'api/admin/contracts/remove-discussion/'+companyid+'/'+userid+'/'+id)
                            .then(response => {
        ...
        const EditMessage = (id) => {
            ...
            axios.get(Globalsettings.url + 'api/admin/contracts/edit-discussion/'+companyid+'/'+userid+'/'+id)
            .then(response => {
    ...
    return (
        ...
        <LoadingOverlay active={isLoading} spinner text='Please Wait...' />
            <div className="mb-4 d-md-flex d-block">
                <NavLink to={`${process.env.PUBLIC_URL}/contract`} className="d-inline-block btn btn_blue"> Back</NavLink>
                                                ...
                                                return (
                                                    <div className="d-flex align-items-center mb-3">
                                                        <div className="d-flex align-items-center mr-auto">
                                                            <div className="mr-3 avatar"><img className="img-gluid" src={val.user.image_url} alt="icon" /></div>
                                                            <div>
                                                                <h4 className="mb-1 fontsize18">{val.user.name}</h4>
                                                                <p className="m-0 paragraph_blue_text_color">{val.message}</p>
                                                            </div>
                                                        </div>
                                                        <Button variant="" onClick={() => EditMessage(val.id)} className="btn py-1 px-2"><img width="20" className="img-fluid" src={edit_2_iconimg} alt="icon" /></Button>
                                                        <Button variant="" onClick={() => DeleteMessage(val.id)} className="btn py-1 px-2"><img width="20" className="img-fluid" src={deleteiconimg} alt="icon" /></Button>
                                                    </div>
                                                )
                                            })
                                            :
