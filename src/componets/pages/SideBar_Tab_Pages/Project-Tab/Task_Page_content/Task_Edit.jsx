...
const Task_Edit = (props) => {
    ...
    useEffect(() => {
        axios.get(Globalsettings.url + 'api/admin/all-tasks/edit/'+props.match.params.id+'/'+companyid+'/'+obj.id)
            .then((response) => {
                ...
                setglobal({ global_Array: response.data.data.setting ? response.data.data.setting : [], });
                setfiles({ files_Array: response.data.data.task.files ? response.data.data.task.files : [], });
                settasklabel({ tasklabel_Array: response.data.data.task.label ? response.data.data.task.label  : [], });
                settaskBoardColumns({ taskBoardColumns_Array: response.data.data.taskBoardColumns ? response.data.data.taskBoardColumns  : [], });
                settitle(response.data.data.task.heading);
                setprojectid(response.data.data.task.project_id);
                settaskcatid(response.data.data.task.task_category_id);
                if(response.data.data.task.description == null){
                    setdescription('');
                }else{
                    setdescription(response.data.data.task.description);
                }
                
                setstartdate(dateFormat(response.data.data.task.start_date,'yyyy-mm-dd'));
                setduedate(dateFormat(response.data.data.task.due_date,'yyyy-mm-dd'));
                if(response.data.data.task.is_private > 0){
                    setis_private(true);
                }
                if(response.data.data.task.billable > 0){
                    setbillable(true);
                }
                if(response.data.data.task.estimate_hours > 0 || response.data.data.task.estimate_minutes > 0){
                    setset_time_estimate(true);
                }
                if(response.data.data.task.dependent_task_id != null){
                    setdependent(true);
                }
                
                setdependent_task_id(response.data.data.task.dependent_task_id);
                setpriority(response.data.data.task.priority);
                setselectedEmpolyee(response.data.data.task.users[0].id);
                setestimate_hours(response.data.data.task.estimate_hours);
                setestimate_minutes(response.data.data.task.estimate_minutes);
                setstatus(response.data.data.task.board_column_id)
                setLoading(false);
            })
            .catch((error) => {
                setLoading(false);
    ...
    const FormSubmitTask = (evt) => {
       ...
       filedata.append('task_id', props.match.params.id);
       filedata.append('file', selectedImage);
        ...
        axios.post(Globalsettings.url + 'api/admin/all-tasks/update/'+props.match.params.id+'/'+companyid+'/'+id, data).then((response) => {
                axios.post(Globalsettings.url + 'api/admin/task-files/'+props.match.params.id+'/'+companyid+'/'+id, filedata).then((response) => {
    ...
    return (
        <>
            ...
            <div className="container-fluid mb-4">
                <h4 className="main_title">Update Task</h4>
            </div>
            {/*  */}
            ...
            <div className="card card_dashboard card-body mb-4 whitecolorbg">
                ...
                <div class="row mb-3">
                    <div className="col-xl-4 col-lg-12">
                            <div className="">
                                <Form.Check name="a" label="Make Private" checked={is_private}  onChange={updateis_private} />
                            </div>
                    </div>
                    <div className="col-xl-4 col-lg-12">
                            <div className="">
                                <Form.Check name="a" label="Billable" checked={billable}  onChange={updatebillable} />
                            </div>
                    </div>
                    <div className="col-xl-4 col-lg-12">
                            <div className="d-flex align-items-center">
                                <div>
                                    <div className="mb-0">
                                        <Form.Check name="a" label="Set time estimate"  checked={set_time_estimate} onChange={updateset_time_estimate} />
                                    </div>
                                </div>
                                {set_time_estimate === true &&
                                <div className="ml-3">
                                    <div class="form-group d-flex align-items-center">
                                            <Form.Control type="number" min="0"  className="w-80px h-40px" required value={estimate_hours} onChange={(e) => setestimate_hours(e.target.value)}  name="estimate_hours"/> <span className="ml-2">Hrs</span>
                                            <Form.Control  type="number" min="0"  className="ml-2 w-80px h-40px" required value={estimate_minutes} onChange={(e) => setestimate_minutes(e.target.value)} name="estimate_minutes"/> <span className="ml-2">Mins</span>
                                    </div>
                                </div>}
                            </div>
                    </div>
                ...
                <div className="col-xl-12 col-lg-12 mb-4">
                    <Form.Check name="dependent" label="Task is dependent on another task" checked={dependent} onChange={updatedependent} />
                </div>   
                {dependent === true &&               
                <div className="col-xl-12 col-lg-12 mb-4">
                ...
                <div className="col-xl-12 col-lg-12 mb-4">
                    ...
                    <Select name="task_labels" options={labellist.labellist_Array.map((val) => { return({ label: val.label_name, value: val.id }) })}  isMulti onChange={handleChange} />
                </div>
                <div className="row">
                ...
                <div className="col-xl-4 col-lg-12 mb-4">
                        <Form.Label className="mb-2 fontsize14 fontweightregular">Status</Form.Label>
                        <Form.Select as="select" required value={status} onChange={(e) => setstatus(e.target.value)}>
                            {taskBoardColumns.taskBoardColumns_Array.map((val) => {
                                return(
                                    <option value={val.id}>{val.column_name}</option>
                                )    
                            })}
                        </Form.Select>
                ...
                <div class="col-lg-12 mb-4" id="list">
                        <ul class="list-group" id="files-list">
                            {files.files_Array.length > 0 ?
                            files.files_Array.map((val) => {
                                return(
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3">
                                                ...
                                                <a target="_blank" href={val.file_url}
                                                    data-toggle="tooltip" data-original-title="View"
                                                    class="btn btn-info btn-circle">View</a>
                                            
                                            <span class="m-l-10">{val.created_at}</span>
                                        </div>
                                    </div>
                                </li>                                    
                                )
