...
const Add_Project = (props) => {
    ...
    useEffect(() => {
        axios.post(Globalsettings.url + 'api/admin/projects/create', {
            ...
            .then((response) => {
                ...
                setdepartments({ departments_Array: response.data.departments ? response.data.departments : [], });
    ...
    const handleSubmit = (evt) => {
        ...
        data.append('image_url', images);
        data.append('companyid', companyid);
        axios.post(Globalsettings.url + 'api/admin/projects/store', data).then((response) => {
            toast.success("Project Successfully Inserted!");
            setLoading(false);
            setTimeout(() => { 
                history.push(`${process.env.PUBLIC_URL}/manage-job`);
            }, 3000);
        }).catch(function (error) {
            toast.success("Some things went wrong!");
        });
        evt.preventDefault();
    }
    ...
    return (
        <>
            ...
            <h4 className="main_title fontsize18 mb-4 px-0">Add New Project</h4>
                ...
                <div className="card_dashboard card card-body whitecolorbg px-4 mb-4">
                    ...
                    <div className="row">
                        ...
                        <div className="col-xl-6 col-lg-12 mb-4">
                            ...
                            <Form.Control name="" className="h-50px border_lightparagraphcolor_1 border-radius-15" required as="select" value={projectcat} onChange={e => setprojectcat(e.target.value)}>
                            ...
                            <div className="d-flex align-items-center">
                                ...
                                {['checkbox'].map((type) => (
                                    <div key={`inline-${type}`} className="mb-0">
                                        <Form.Check className="fontsize14 fontweightregular" inline label="Without deadline" type={type} id={`inline-${type}-1`} />
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="col-xl-6 col-lg-12 mb-4">
                            <Form.Label className="mb-2 fontsize14 fontweightregular">Add Job Members <NavLink onClick={() => setModalShowAssign(true)} to="#" className="blusecolorbg p-1 border-radius-5 ml-2"><img className="img-fluid" src={plusiconimg} alt="" /></NavLink></Form.Label>
                            <Form.Control name="" className="h-50px border_lightparagraphcolor_1 border-radius-15" as="select" value={jobmember} onChange={e => setjobmember(e.target.value)}>
                                <option>Choose Members</option>
                                {memberslist.memberslist_Array.map((val) => {
                                    return (
                                        <option value={val.id}>{val.name}{val.id === userid ? ' (You)' : ''}</option>
                                    )
                                })}
                            </Form.Control>
                        </div>
                ...
                <div className="card_dashboard card card-body whitecolorbg px-4">
                    ...
                    <div className="row">
                        ...
                        <div className="col-lg-12">
                            ...
                            <Button variant="" className="w-100px bodycolorbg btn">Reset</Button>
                        </div>
                    </div>
