...
const Add_New_Client = () => {
    ...
    useEffect(async () => {
        await axios.get(Globalsettings.url + 'api/admin/clients/create/' + companyid)
            .then((response) => {
    ...
    const DeleteClientCategory = (id) => {
        setLoading(true);
        axios.get(Globalsettings.url + 'api/admin/clientCategory/destroy/'+id)
            .then(response => {
                toast.success("Client Category Deleted Successfully");
                setLoading(false);
                setclientcatarray({ clientcatarray_Array: clientcatarray.clientcatarray_Array.filter(item => item.id !== id) });
            });
    }
    // Delete Client sub Category
    const DeleteClientSubCategory = (id) => {
        setLoading(true);
        axios.get(Globalsettings.url + 'api/admin/clientSubCategory/destroy/'+ id)
            .then(response => {
                toast.success("Client Sub Category Deleted Successfully");
                setLoading(false);
                setclientsubcatarray({ clientsubcatarray_Array: clientsubcatarray.clientsubcatarray_Array.filter(item => item.id !== id) });
            });
    }
                                    ...
                                    {clientcatarray.clientcatarray_Array.map((val, index) => {
                                        ...
                                        return (
                                            <tr>
                                                ...
                                                <td><NavLink to="#" onClick={() => DeleteClientCategory(val.id)} className="border_lightredcolor_1 py-1 px-3 redcolortext fontsize14">Remove</NavLink></td>
                                            </tr>
                                        )
                                    })}
                                </tbody>
                                    ...
                                    {clientsubcatarray.clientsubcatarray_Array.map((val, index) => {
                                        ...
                                        return (
                                            <tr>
                                                <td>{val.number}</td>
                                                <td>{val.category_name}</td>
                                                <td><NavLink to="#" onClick={() => DeleteClientSubCategory(val.id)} className="border_lightredcolor_1 py-1 px-3 redcolortext fontsize14">Remove</NavLink></td>
                                            </tr>
                                        )
                                    })}
                                </tbody>
