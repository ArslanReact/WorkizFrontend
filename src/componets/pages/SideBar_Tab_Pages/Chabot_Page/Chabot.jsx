...
const Chabot = (props) => {
    ...
    useEffect(() => {
        axios.get(Globalsettings.url+'api/admin/user-chat/'+id+'/'+companyid)
        .then((response) =>{
    ...
    const StartConversation = () => {
        setactive(true);
        axios.get(Globalsettings.url+'api/admin/user-chat/create/'+id+'/'+companyid)
        .then((response) =>{
            setmessageSetting({messageSetting_Array: response.data.messageSetting ? response.data.messageSetting : [],}); 
            setempList({empList_Array: response.data.members ? response.data.members : [],}); 
    ...
    const HandleMessageSend = (evt) => {
        var utype = '';
        if(status == 1) { utype = "employee"; }else{ utype = "client"; }
        axios.post(Globalsettings.url + 'api/admin/message-submit', { 
            companyid:companyid,
            from_user_id:id,
            user_id: member,
            message: message,
    ...
    const SendMessage = (evt) => {
        ...
        axios.post(Globalsettings.url + 'api/admin/message-submit', { 
            ...
            setUsersList({UsersList_Array: response.data.userList ? response.data.userList : [],});
            var url = Globalsettings.url+'api/admin/user-chat/'+id+'/'+companyid;
            setenteredMassge('');
            $.ajax({
                type: 'GET',
                url: url,
            ...
            <Modal show={modalShowClientCategory} onHide={() => setModalShowClientCategory(false)} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
                ...
                <Modal.Body className="p-0 mt-4">
                    <div class="table-sm-responsive">
                            ...
                            {messageSetting.messageSetting_Array.allow_client_admin === "yes" &&
                                <>
                                <div className="mb-4 align-self-end col-4">
                                    <div className="d-flex align-items-center">
                                        ...
                                        <div className="ml-2">
                                            <Form.Check type="radio" name="release" className="d-flex align-items-center" aria-label="radio 3" label="Client" checked={status === 2} onClick={(e) => setStatus(2)} />
                                        </div>
                                    </div>
                                </div>
                                </>
                            ...
                            {messageSetting.messageSetting_Array.allow_client_admin === "yes" &&
                                ...
                                {status === 2 &&
                                    <div className="form-group mb-4">
                                        <Form.Control className="transparent_form h-45px" as="select" required value={client} onChange={e => setclient(e.target.value)}>
                                            <option value="">Select Client</option>
                                            {clientList.clientList_Array.map((val)=>{
                                                return(
                                                    <option value={val.id}>{val.name}</option>
                                                )  
                                            })}
